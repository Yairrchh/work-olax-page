...
const ProductsListHome = ({data}) => {
    ...
    return (
    <div className="flex  flex-col justify-center rounded-xl items-center py-10 px-8 transition-transform transform-gpu hover:scale-110 ease-out duration-300" style={{ backgroundImage: `url(${data.background})`, backgroundSize: 'cover'}}>
        <h1 className="text-2xl font-extrabold text-center text-white">{data.name}</h1>
        <img className="max-w-80 max-h-80" src={data.image} />
